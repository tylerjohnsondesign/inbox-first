...
function ifklicked_schedule_send_campaign($schedule) {
    ...
    // Get.
    $html       = $opts['template_schedule_' . $schedule];

    // Check.
    if( $html === 'id-custom' ) {

        // Get URL.
        $html = file_get_contents( $opts['customurl_schedule_' . $schedule] );
        ...
        include TEMPKLICKED_BASE_PATH . 'templates/' . $base . '.php';
...
function ifklicked_send_campaign() {
    ...
    $url        = IFKLICKED_IBF_URL.'/v2/mailing_lists/'.$list.'/campaigns';
    $template   = $_POST['temp'];
    $html       = get_permalink(str_replace('id-', '', $_POST['url'])).'?cache=busting';
    $date       = $_POST['date'];
    $settime    = $_POST['time'];
